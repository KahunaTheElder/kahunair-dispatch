...
class TaxiGraphService {
    ...
    _onFacilityDataEnd(recv) {
        ...
        // Bi-directional edge map: 'startIdx_endIdx' → TAXI_NAME array index
        const edgeMap = new Map();
        for (const path of taxiPaths) {
            edgeMap.set(`${path.start}_${path.end}`, path.nameIndex);
            edgeMap.set(`${path.end}_${path.start}`, path.nameIndex);
        }
    ...
    _resolveRoute(graph, siWaypoints) {
        ...
        for (let i = 0; i < matched.length - 1; i++) {
            const ni = edgeMap.get(`${matched[i]}_${matched[i + 1]}`);
            if (ni !== undefined) segNames.push(taxiNames[ni]?.name || '');
        }
